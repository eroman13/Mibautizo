...
export async function autoSeedIfEmpty() {
  try {
    ...
    // Backfill de textos: renombra cualquier mención a "gemela(s)/gemelar"
    // guardada en la BD por "melliza(s)/doble" (mensaje de bienvenida y regalos).
    // Idempotente: solo actualiza filas que aún contengan los términos antiguos.
    const reemplazarTerminos = (texto: string) =>
      texto
        .replace(/gemelar/gi, 'doble')
        .replace(/gemelas/gi, 'mellizas')
        .replace(/gemela/gi, 'melliza');

    const actualizarTerminosViejos = async () => {
      const evento = await prisma.event.findFirst();
      if (evento && /gemel/i.test(evento.mensajeBienvenida)) {
        await prisma.event.update({
          where: { id: evento.id },
          data: { mensajeBienvenida: reemplazarTerminos(evento.mensajeBienvenida) },
        });
        console.log('✅ Mensaje de bienvenida actualizado: gemelas → mellizas');
      }
      ...
      for (const gift of gifts) {
        const nuevoNombre = reemplazarTerminos(gift.nombre);
        const nuevaDescripcion = reemplazarTerminos(gift.descripcion);
        if (nuevoNombre !== gift.nombre || nuevaDescripcion !== gift.descripcion) {
      ...
      if (actualizados > 0) {
        console.log(`✅ ${actualizados} regalo(s) actualizado(s): gemelas → mellizas`);
      }
    };

    // Si ya existe un evento, la BD ya fue sembrada. No hacer nada.
    const existingEvent = await prisma.event.findFirst();
    await actualizarTerminosViejos();
    if (existingEvent) {
    ...
    if (giftCount === 0) {
      const regalos = [
        { nombre: 'Pack de 4 bodies de algodón', descripcion: '2 bodies para cada melliza, 100% algodón suave, tallas 3-6 meses', precioCLP: 18000, imagenUrl: 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=600', permiteColaborativo: false },
        { nombre: 'Coche doble para mellizas', descripcion: 'Coche doble liviano, con capota ajustable y frenos de seguridad', precioCLP: 180000, imagenUrl: 'https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?w=600', permiteColaborativo: true },
        ...
        { nombre: '2 mamaderas anticólicos', descripcion: 'Kit de 4 mamaderas (2 para cada melliza) con sistema anticólicos', precioCLP: 22000, imagenUrl: 'https://images.unsplash.com/photo-1587735243474-306e9e1c3c6c?w=600', permiteColaborativo: false },
        ...
        { nombre: 'Pack de 4 pijamas de invierno', descripcion: '2 pijamas para cada melliza, abrigaditos para el invierno', precioCLP: 28000, imagenUrl: 'https://images.unsplash.com/photo-1519238263530-99bdd11df2ea?w=600', permiteColaborativo: false },
        ...
        { nombre: '2 peluches de regalo', descripcion: 'Ositos de peluche suaves, uno para cada melliza', precioCLP: 20000, imagenUrl: 'https://images.unsplash.com/photo-1530325553241-4f6e7690cf36?w=600', permiteColaborativo: false },
        ...
        { nombre: 'Aporte libre para las mellizas', descripcion: 'El monto que tú elijas para ayudarnos con lo que las pequeñas necesiten 💝', precioCLP: 0, imagenUrl: 'https://images.unsplash.com/photo-1515488042361-ee00e0ddd4e4?w=600', permiteColaborativo: true },
      ];
